...
pub fn generate_password(length: u32, digits: u32, specials: u32) -> String {
    ...
    for _ in 0..digits {
        result.push(DIGITS.chars().choose(&mut rng).unwrap_or_default());
    }

    for _ in 0..specials {
        result.push(SPECIAL.chars().choose(&mut rng).unwrap_or_default());
    }

    for _ in 0..(length - digits - specials) {
        result.push(ALL.chars().choose(&mut rng).unwrap_or_default());
    }
